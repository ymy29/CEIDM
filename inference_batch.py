...
from trainer_only_kg import read_official_ckpt, batch_to_device
import numpy as np
import clip
...
class DynamicFusionConv(nn.Module):
    ...
    def forward(self, x):
        # Generate dynamic weights based on input features
        dynamic_weight = self.base_conv.weight * (1 + 0.1 * x.mean()) 
        return F.conv2d(
            x,
            dynamic_weight,
...
def load_ckpt(ckpt_path):
    ...
    #         torch.nn.init.xavier_normal_(module.fusion_conv.weight)
    #         torch.nn.init.zeros_(module.fusion_conv.bias)

    # replace original fusion_conv
    for name, module in model.named_modules():
        if isinstance(module, BasicTransformerBlock) and isinstance(module.fusion_conv, nn.Conv2d):
            module.fusion_conv = DynamicFusionConv(module.fusion_conv)
...
if __name__ == "__main__":
    ...
    meta_list = [
        ...
        dict(
            ...
            save_folder_name="../DATA/generation_hoi"
        ),
    ]
