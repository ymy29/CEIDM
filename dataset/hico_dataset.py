...
class HICODataset(HOIBaseDataset):
    def __init__(self,
        ...
        # Load tsv data
        #print(f"Loading files from: {os.path.join(self.dataset_path, 'embed_*.clip.pt')}")
        #self.files = glob.glob(os.path.join(self.dataset_path, 'embed_*.clip.pt'))
        #print(f"Found {len(self.files)} files: {self.files}")
        self.files = glob.glob(os.path.join(self.dataset_path, 'embed_*.clip.pt'))
        ...
        if self.max_images is not None:
            self.files = self.files[:self.max_images] 
