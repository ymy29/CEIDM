...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    # def forward(self, x, emb, context, objs, kgs):
    #     for layer in self:
    #         if isinstance(layer, TimestepBlock):
    #             x = layer(x, emb)
    #         elif isinstance(layer, HOISpatialTransformer):
    #             x = layer(x, context, objs, kgs)
    #         else:
    #             x = layer(x)
    #     return x

    def forward(self, x, emb, context, objs, objs1, kgs, subject_boxes, action_boxes, object_boxes, subject_positive_embeddings, object_positive_embeddings, timesteps):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, HOISpatialTransformer):
                x = layer(x, context, objs, objs1, kgs, subject_boxes, action_boxes, object_boxes, subject_positive_embeddings, object_positive_embeddings, timesteps)
            else:
                x = layer(x)
        return x
...
class HOIUNetModel(nn.Module):
    ...
    def forward(self, input):
        ...
        #objs,subject_boxes, object_boxes, action_boxes = self.position_net(**grounding_input)
        objs, objs1, subject_boxes, action_boxes, object_boxes, subject_positive_embeddings, object_positive_embeddings = self.position_net(**grounding_input)
        ...
        if self.inpaint_mode:
            if self.downsample_net is not None:
                raise NotImplementedError("Inpaint mode with downsampler not implemented")
            h = torch.cat([h, input["inpainting_extra_input"]], dim=1)
        ...
        for module in self.input_blocks:
            h = module(h, emb, context, objs, objs1, kgs, subject_boxes, action_boxes, object_boxes, subject_positive_embeddings, object_positive_embeddings, timesteps)
            hs.append(h)

        h = self.middle_block(h, emb, context, objs, objs1, kgs, subject_boxes, action_boxes, object_boxes, subject_positive_embeddings, object_positive_embeddings, timesteps)
